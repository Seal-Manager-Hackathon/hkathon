...
## Nghiệp vụ
- Chỉ lấy các bản ghi `AssignEvents` mà user hiện tại có **`EventRole = Staff`**, **đang active** (`AssignEvents.IsDisable = false`), và **event không bị disable** (`Event.IsDisable = false`).
