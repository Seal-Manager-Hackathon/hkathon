# GET /api/v1/student/rounds/{roundId}/criteria-templates

> Student lấy danh sách criteria template đang active của 1 round.

**Controller:** [StudentCriteriaController.cs](Controllers/Student/StudentCriteriaController.cs)

## Nghiệp vụ
- Lấy danh sách criteria template của 1 round.
- **Chỉ lấy template đang active** (`IsActive = true`) và **ko bị disable** (`IsDisable = false`).
- Hỗ trợ tìm kiếm theo title (contains, ko phân biệt hoa thường).
- Sắp xếp theo CreatedAt giảm dần.
- 404 nếu roundId ko tồn tại.

## Phân quyền
- ✅ Student

## Request
| Param | Kiểu | Bắt buộc | Ví dụ |
|-------|------|----------|-------|
| roundId | guid | ✅ (route) | `3fa85f64-5717-4562-b3fc-2c963f66afa6` |
| Keyword | string | ❌ (query) | `Đánh giá` |
| PageIndex | int | ❌ (query) | `1` |
| PageSize | int | ❌ (query) | `10` |

## Response (200)
```json
{
  "data": {
    "templates": [
      {
        "id": "guid",
        "roundId": "guid",
        "title": "Tiêu chí đánh giá vòng 1",
        "description": "...",
        "isDisable": false,
        "isActive": true,
        "createdAt": "2026-07-07T12:00:00Z",
        "updatedAt": "2026-07-07T12:00:00Z"
      }
    ],
    "totalCount": 1,
    "pageIndex": 1,
    "pageSize": 10
  },
  "message": "Fetched Successfully",
  "error": null,
  "isSuccess": true,
  "status": 200,
  "traceId": "00-abc123...",
  "timestampUtc": "2026-07-07T12:00:00Z"
}
```

## Lỗi
| Status | message | Khi nào | FE xử lý |
|--------|---------|---------|----------|
| 401 | Invalid Or Expired Token | Token hết hạn/thiếu | Redirect login |
| 403 | You do not have permission to perform this action | Không phải Student | Ẩn chức năng |
| 404 | Resource Not Found | roundId ko tồn tại | Báo "Không tìm thấy vòng" |

> **Ref:** [Admin API tương ứng](/api/v1/admin/rounds/{roundId}/criteria-templates)
