# GET /api/v1/student/events/{eventId}/awards

> Student lấy danh sách awards của event, phân trang, có thể search theo tên.
...
- Giống Admin API nhưng **chỉ lấy award có IsDisable = false** (ko lấy giải đã bị xóa mềm).
- Ko hỗ trợ lọc `isDisable` vì student chỉ thấy award active.
- Sắp xếp theo LevelAward tăng dần: giải Nhất (level 1) lên trên cùng.
- Có thể lọc `keyword`: tìm kiếm theo tên award (contains, ko phân biệt hoa thường).
...
| 404    | Resource Not Found                                | eventId ko tồn tại      | Báo "Không tìm thấy sự kiện"        |
