# GET /api/v1/student/teams/{teamId}/members

> Student lấy danh sách thành viên của team (chỉ member active), kèm total, totalDisable, phân trang.

**Controller:** [StudentTeamController.cs](Controllers/Student/StudentTeamController.cs)

> **Ref:** [Admin API tương ứng](/api/v1/admin/teams/{teamId}/members)

## Nghiệp vụ
- Giống Admin API nhưng **chỉ lấy member có IsDisable = false và Status = Active**.
- Ko lấy member có Status = Inactive (đã rời team).
- `totalCount`: số member active trong page.
- `totalDisable`: số member bị disable hoặc inactive (tính trên tổng số, ko phải trên page).
- Phân trang: mặc định pageIndex=1, pageSize=10.
- 404 nếu teamId không tồn tại hoặc team bị disable.
...
| 404 | Team Not Found | teamId ko tồn tại hoặc bị disable | Báo "Team không tồn tại" |
