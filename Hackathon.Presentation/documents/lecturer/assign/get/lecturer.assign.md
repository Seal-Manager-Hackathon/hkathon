# GET /api/v1/lecturer/events/{eventId}/assign

> Lecturer đã đăng nhập lấy danh sách user được phân công trong một event (Staff, Judge, Mentor), chỉ các bản ghi active, có filter và phân trang.

## Nghiệp vụ

Lecturer muốn xem danh sách tất cả user được phân công vào event mà họ tham gia. Kết quả trả về bao gồm thông tin user, event role (Staff/Judge/Mentor), các track được gán.

- **Chỉ lấy các bản ghi active** (`IsDisable = false`).
- Mỗi user có thể được gắn vào nhiều track.
- Lecturer **phải được phân công** vào event đó thì mới xem được.
- Response giống hệt Admin `GET /api/v1/admin/assign/events/{eventId}/assigned`.
- Hỗ trợ tìm kiếm theo **email** hoặc **tên**, lọc theo **EventRole**.

## Phân quyền
- ✅ Lecturer (RoleEnum = Lecturer) — phải được phân công vào event tương ứng

## Request

### Route Parameters
| Parameter | Type | Bắt buộc | Ví dụ | Ghi chú |
|-----------|------|----------|-------|---------|
| eventId | Guid | Có | 3fa85f64-5717-4562-b3fc-2c963f66afa6 | ID của event |

### Query Parameters
| Parameter | Type | Bắt buộc | Mặc định | Ghi chú |
|-----------|------|----------|---------|---------|
| Keyword | string | Không | - | Tìm kiếm theo email hoặc fullname |
| EventRole | string | Không | - | Lọc theo event role: `Mentor`, `Judge`, `Staff` |
| PageIndex | int | Không | 1 | Trang hiện tại |
| PageSize | int | Không | 10 | Số lượng item mỗi trang (max 100) |

## Response (200)
```json
{
  "data": {
    "items": [
      {
        "assignEventId": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
        "userId": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
        "email": "user@example.com",
        "firstName": "Nguyen",
        "lastName": "Van A",
        "avatarUrl": "https://example.com/avatar.jpg",
        "eventRole": "Judge",
        "isDisable": false,
        "assignTracks": [
          {
            "trackId": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
            "title": "Tri tue nhan tao",
            "eventId": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
            "isDisable": false
          }
        ]
      }
    ],
    "totalCount": 5,
    "pageIndex": 1,
    "pageSize": 10
  },
  "message": "Fetched Successfully",
  "error": null,
  "isSuccess": true,
  "status": 200,
  "traceId": "00-abc123...",
  "timestampUtc": "2026-07-07T12:00:00Z"
}
```

## Lỗi
| Status | message | Khi nào | FE xử lý |
|--------|---------|---------|----------|
| 400 | Invalid EventRole | EventRole không hợp lệ | Hiển thị thông báo lỗi |
| 400 | PageIndex/PageSize invalid | PageIndex hoặc PageSize không hợp lệ | Hiển thị thông báo lỗi |
| 401 | Invalid Or Expired Token | Token hết hạn/thiếu | Chuyển về trang login |
| 403 | You Are Not Assigned to This Event | Lecturer chưa được phân công vào event | Hiển thị thông báo Không có quyền |
| 403 | You do not have permission to perform this action | Không phải Lecturer | Hiển thị thông báo Không có quyền |

> **Ref:** [Admin API tương ứng](/api/v1/admin/assign/events/{eventId}/assigned)
