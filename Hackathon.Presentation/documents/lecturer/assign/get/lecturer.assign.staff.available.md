# GET /api/v1/lecturer/events/{eventId}/staff/available

> Lecturer lấy danh sách Staff chưa được phân công vào event (không bị ban/disable).

**Controller:** [LecturerAssignController.cs](Controllers/Lecturer/LecturerAssignController.cs)

## Nghiệp vụ

**Router:** `GET /api/v1/lecturer/events/{eventId}/staff/available`

- Giống hệt Admin `GET /api/v1/admin/assign/events/{eventId}/staff/available`, khác auth là Lecturer.
- Chỉ trả về user có role = `Staff`.
- Tự động loại bỏ user đã bị **disable** (`IsDisable = true`) hoặc đã bị **ban** (`BanReason != null`).
- Chỉ lấy Staff chưa được assign vào event này (tránh duplicate).
...
> **Ref:** [Admin API tương ứng](/api/v1/admin/assign/events/{eventId}/staff/available)
